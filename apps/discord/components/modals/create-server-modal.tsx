    ...
    <Dialog open={isModalOpen} onOpenChange={() => onClose()}>
      <DialogContent className="bg-discord p-0 rounded-[3px]">
        <DialogHeader className="mt-8 w-full flex flex-col items-center">
          <DialogTitle className="text-2xl font-semibold">Create your own Server</DialogTitle>
          <DialogDescription className="px-12 mt-3 text-center">Give your server a personality with a name and an image. You can always change it later.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form className="" onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="mx-5 mb-3 rounded-[3px]">
                  <FormLabel className="uppercase font-medium text-white text-opacity-20">Server Name</FormLabel>
                  <FormControl>
                    <Input className="bg-[#202225] rounded-[3px] outline-none focus-visible:ring-offset-0 focus-visible:ring-0" {...field} placeholder="Server Name" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <DialogFooter className="bg-[#282b30] px-6 py-4">
              <Button type="submit" className="bg-discord_purple rounded-[2px] text-white hover:bg-discord_purple" disabled={isSubmitting}>
                Create
              </Button>
            </DialogFooter>
          </form>
