...
const ServerSidebar = async ({ server_id }: { server_id: string }) => {
  ...
  const data = await getServer(server_id, profile?.id)
  //console.log(data)
  ...
  return (
    <div className='min-w-[220px] max-w-[220px] h-screen overflow-auto bg-[rgb(40,43,48)] text-white'>
      <ServersidebarNavbar label={data.server.name} className="" member={data.member} />
      <ServerSearch data={[
        {
          label: 'Channels',
          type:  'channel',
          data:  data.categories.flatMap((category) => category.channels.map((channel) => ({
            id:   channel.id,
            name: channel.name,
            icon: channel.channel_type === channelTypesEnum.TEXT ? <Hash /> : channel.channel_type === channelTypesEnum.VOICE ? <Speaker /> : <Video />
          })))
        },
        {
          label: 'Members',
          ...
          data: [
            {
              id: data.member.id,
              name: 'Sarthak Roy',
              icon: data.member.role === 'admin' ? <Shield /> : data.member.role === 'moderator' ? <ShieldHalf /> : null
            }
          ]
