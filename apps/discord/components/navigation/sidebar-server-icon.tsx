...
import AokuraConnectIcon from "../logo/connect";
  ...
  return (
    <TooltipWrapper label={server.name} side="right">
      ...
      >
        <div
          className={cn(
            "bg-white rounded-r-sm w-1 transition-all duration-200",
            params!.serverId !== server.id && "h-3 group-hover:h-10",
            params!.serverId === server.id && "h-10"
          )}
        />
        <div
          className={cn(
            "w-11 h-11 overflow-hidden transition-all",
