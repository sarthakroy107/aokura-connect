...
const Page = async ({ params }: { params: { serverId: string, channelId: string } }) => {
  ...
  return (
    <div className="w-full bg-discord h-full">
      <ChannelNavbar id={channel.name} name={channel.name} type={channel.type} />
      <ChatMessages channel_id={params.channelId} />
      {
        channel.type === 'text' && (
          <>
            <ChatInput serverId={params.serverId} channelId={params.channelId} type="channel" name={channel.name} />
          </>
        )
      }
    </div>
